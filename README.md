...
A diagnostic and reference application for deploying Cloud Run services behind Google Cloud's [Identity-Aware Proxy (IAP)](https://cloud.google.com/iap/docs/concepts-overview). Use it to learn how IAP works, verify your configuration, and as a starting point for your own IAP-protected services.
...
That's it. The script enables APIs, builds from source, deploys to Cloud Run, enables IAP, restricts access to `@myowndomain.com`, and sets the correct `IAP_AUDIENCE` environment variable.
...
└── internal/
    ...
    ├── components/
    │   ├── layout.templ             # Page layout + navigation
    ...
    ├── render/
    │   └── render.go                # Content negotiation (HTML/JSON)
    └── reqlog/
        └── ringbuffer.go            # Thread-safe circular buffer (200 entries)
```
