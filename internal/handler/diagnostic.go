...
func Diagnostic(verifier *iap.Verifier) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var checks []components.Check

		// Check 1: JWT present?
		if det.HasJWT {
			checks = append(checks, components.Check{
				Name:   "JWT Present",
				Status: "pass",
		...
		data := components.DiagnosticData{Checks: checks}
		...
		if err := components.DiagnosticPage(data).Render(r.Context(), w); err != nil {
